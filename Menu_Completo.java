...
static String[] productos = {
    "Manzana (unidad)", "Naranja (unidad)", "Plátano (unidad)", "Pera (unidad)", "Uva (racimo)", "Mango (unidad)", "Fresa (caja)", // Frutas
    "Lechuga (unidad)", "Tomate (unidad)", "Zanahoria (unidad)", "Pepino (unidad)", "Papa (unidad)", "Cebolla (unidad)", "Ajo (cabeza)", // Verduras
    "Pollo (libra)", "Carne de res (libra)", "Cerdo (libra)", "Pescado (libra)", "Camarones (libra)", "Salchichas (paquete)", "Huevos (docena)", // Carnes
    "Leche (litro)", "Yogur (litro)", "Queso (libra)", "Mantequilla (libra)", "Crema (litro)", "Helado (litro)", "Leche condensada (lata)", // Lácteos
    "Arroz (libra)", "Pasta (paquete)", "Pan (unidad)", "Harina (libra)", "Azúcar (libra)", "Sal (libra)", "Aceite (litro)" // Granos y cereales
}; 
static int[] cantidades = {
    10, 20, 15, 12, 25, 18, 30, // Frutas
    20, 15, 25, 18, 30, 22, 10, // Verduras
    10, 8, 12, 15, 10, 20, 30, // Carnes
    20, 15, 25, 18, 30, 22, 10, // Lácteos
    50, 40, 30, 25, 35, 45, 20 // Granos y cereales
}; 
static double[] precios = {
    1200, 800, 600, 1000, 3500, 2000, 5000, // Frutas (COP)
    2000, 500, 300, 700, 400, 600, 1200, // Verduras (COP)
    6000, 12000, 8000, 10000, 18000, 8000, 7000, // Carnes (COP)
    3500, 4000, 7500, 9000, 10000, 12000, 5000, // Lácteos (COP)
    2000, 3500, 2000, 1500, 1200, 800, 7000 // Granos y cereales (COP)
}; 
static String[] categorias = {
    "Frutas", "Frutas", "Frutas", "Frutas", "Frutas", "Frutas", "Frutas", // Frutas
    "Verduras", "Verduras", "Verduras", "Verduras", "Verduras", "Verduras", "Verduras", // Verduras
    "Carnes", "Carnes", "Carnes", "Carnes", "Carnes", "Carnes", "Carnes", // Carnes
    "Lácteos", "Lácteos", "Lácteos", "Lácteos", "Lácteos", "Lácteos", "Lácteos", // Lácteos
    "Granos y cereales", "Granos y cereales", "Granos y cereales", "Granos y cereales", "Granos y cereales", "Granos y cereales", "Granos y cereales" // Granos y cereales
}; 
static String[] codigos = {
    "001", "002", "003", "004", "005", "006", "007", // Frutas
    "008", "009", "010", "011", "012", "013", "014", // Verduras
    "015", "016", "017", "018", "019", "020", "021", // Carnes
    "022", "023", "024", "025", "026", "027", "028", // Lácteos
    "029", "030", "031", "032", "033", "034", "035" // Granos y cereales
}; 
    ...
    public static void main(String[] args) {
        ...
        while (continuar) {
            ...
            System.out.println(ANSI_GREEN + "7. Cambiar precio de producto" + ANSI_RESET);
            System.out.println(ANSI_GREEN + "8. Salir" + ANSI_RESET);
            System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
            ...
            switch (opcion) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    cambiarPrecioProducto(scanner); // Llamar al método para cambiar el precio de un producto
                    break;
                case 8:
...
public static void mostrarInventario(Scanner scanner) {
    ...
    for (String categoria : categorias) {
        if (!categoriasUnicas.contains(categoria.toLowerCase())) {
            categoriasUnicas.add(categoria.toLowerCase());
            System.out.println(ANSI_YELLOW + "- " + capitalizarPrimeraLetra(categoria) + ANSI_RESET);
        }
    }
    System.out.print(ANSI_YELLOW + "Elige una categoría: " + ANSI_RESET);
    String categoriaElegida = scanner.nextLine().toLowerCase();
    if (!categoriasUnicas.contains(categoriaElegida)) {
        System.out.println(ANSI_RED + "Categoría no encontrada." + ANSI_RESET);
        return;
    }
    System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
    System.out.println(ANSI_GREEN + "Productos en la categoría " + capitalizarPrimeraLetra(categoriaElegida) + ":" + ANSI_RESET);
...
public static String capitalizarPrimeraLetra(String texto) {
    ...
    public static void realizarVenta(Scanner scanner) {
        ...
        while (continuarVenta) {
            ...
            if (cantidadAVender <= 0) {
                System.out.println(ANSI_RED + "Cantidad no válida. Debe ser mayor que cero." + ANSI_RESET);
                System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
                return;
            }
    ...
    public static void cambiarPrecioProducto(Scanner scanner) {
        System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
        System.out.println(ANSI_PURPLE + "       CAMBIAR PRECIO PRODUCTO     " + ANSI_RESET);
        System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
        System.out.print(ANSI_YELLOW + "Código del producto: " + ANSI_RESET);
        String codigo = scanner.nextLine(); // Leer el código del producto
        int indice = buscarProductoPorCodigo(codigo); // Buscar el índice del producto
        ...
        System.out.print(ANSI_YELLOW + "Nuevo precio: " + ANSI_RESET);
        String precioStr = scanner.next().replace(",", "."); // Permitir tanto "," como "."
        double nuevoPrecio = Double.parseDouble(precioStr); // Convertir el precio a double
        scanner.nextLine(); // Consumir la nueva línea

        precios[indice] = nuevoPrecio; // Actualizar el precio del producto

        System.out.println(ANSI_GREEN + "Precio actualizado." + ANSI_RESET);
        System.out.println(ANSI_CYAN + "===================================" + ANSI_RESET);
    }
}
